<template>
  <div>
    <el-table :data="permissionArr" style="width: 100%; margin-bottom: 20px" row-key="id" border>
      ...
      <el-table-column label="操作" sortable>
        <template #="{ row, $index }">
          ...
          <el-popconfirm :title="`Are you sure to delete ${row.name}?`" width="260px" @confirm="deleteMenu(row)">
            <template #reference>
    ...
    <el-dialog v-model="dialogVisible" :title="menuParams.id ? '更新菜单' : '添加菜单'" width="30%">
      <!-- :model="form" ref="form" :rules="rules" label-width="80px" :inline="false" size="normal" -->
      <el-form label-width="80px">
        <el-form-item label="名称">
          <el-input v-model="menuParams.name" placeholder='请你输入菜单名称...'></el-input>
        </el-form-item>
        <el-form-item label="权限">
          <el-input v-model="menuParams.code" placeholder="请你输入权限值..."></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
...
// ts type
import type { PermissionResponseData, Permission, MenuData } from '@/api/acl/menu/type'
